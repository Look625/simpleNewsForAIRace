  ...
  <div class="video-page min-h-screen bg-gray-50" @scroll="handleScroll">
      ...
      <div class="scrollable-container" @scroll="handleScroll">
        ...
        <div v-else>
          <div class="grid grid-cols-2 gap-4">
            <div v-for="video in filteredVideos" :key="video.id" class="video-item bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow cursor-pointer" @click="goToVideo(video.id)">
              <h2 class="text-lg font-semibold p-4 border-b">{{ video.title }}</h2>
              <p class="text-sm text-gray-600 p-4">{{ video.description }}</p>
              <div class="flex justify-between p-4">
                <span class="text-sm text-gray-500">浏览: {{ video.views }}</span>
                <span class="text-sm text-gray-500">点赞: {{ video.likes }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <footer class="fixed bottom-0 left-0 right-0 bg-white shadow-md">
      <div class="flex justify-around p-4">
        <router-link to="/" class="text-gray-600">首页</router-link>
        <router-link to="/video" class="text-blue-500">视频</router-link>
        <router-link to="/profile" class="text-gray-600">个人中心</router-link>
      </div>
    </footer>
  </div>
</template>
