<template>
  <div class="video-page min-h-screen bg-gray-50" @scroll="handleScroll">
    ...
    <!-- 内容区域 -->
    <main class="p-4" ref="contentArea">
      <div v-if="isLoading" class="loading-indicator">加载中...</div>
      <div v-else>
        <div class="space-y-4">
          <div v-for="video in videoItems" :key="video.id" class="video-item">
            <!-- 视频项内容 -->
            <h2>{{ video.title }}</h2>
...
const loadVideos = async () => {
  ...
  const newVideos = await getVideosByCategory('all', page.value) // 假设这个函数可以接受分页参数
  if (newVideos.length) {
    videoItems.value.push(...newVideos)
    page.value++
  } else {
    hasMore.value = false
  }
